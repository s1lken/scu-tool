...
bool backdoor_init(void) {
    ...
    while (connection_ptr > (void *) MEMORY_END) {
        ...
        if (!memcmp(connection_ptr, MAGIC_STR, sizeof(MAGIC_STR))) {
            board_print(2, "Connection with backdoor client established at addr: %p!\n", connection_ptr);
        ...
        /* Go to next HugeTLB address. If HugeTLB is not enabled, this needs to be changed and finding the client will take a lot longer */
